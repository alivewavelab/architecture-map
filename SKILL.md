...
复制 [scripts/validate-module-file-map.mjs](scripts/validate-module-file-map.mjs) 到项目 `tooling/arch-module-graph/`，**只改顶部 CONFIG**：
...
- 新脚本：`GRAPH_DEPTH=all` 时必须进图（D `files` 或先跑生成器进 `#generated-graph`），`io` 必须读代码后写成人类话，禁止长期留「待读代码填写」
